...
//! - **Blazingly Fast Reading of XTC Files (`molly`)**: Enables the use of a pioneering [`molly`](https://crates.io/crates/molly) crate for very fast reading of xtc files. (Currently an experimental feature.) With `molly`, iterating through XTC files is ~50% faster than without this flag and ~60% faster than in MDAnalysis.
//!
