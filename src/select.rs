...
fn get_macros() -> HashMap<&'static str, &'static str> {
    ...
    macros.insert(
        "@protein",
        "(resname ALA ARG ASN ASP CYS GLU GLN GLY HIS ILE LEU LYS MET PHE PRO SER THR TRP TYR VAL)",
    );
    macros.insert(
        "@water",
        "(name W OW HW1 HW2 OH2 H1 H2 or resname W WAT SOL TIP3 TIP4 SPC SPCE)",
    );
    macros.insert("@ion", "(name NA NA+ CL CL- K K+ SOD CLA CA CA2+)");
...
mod pass_tests {
    ...
    parsing_success!(
        ...
        "@ion",
        Select::AtomName(
            vec!["NA", "NA+", "CL", "CL-", "K", "K+", "SOD", "CLA", "CA", "CA2+"]
                .iter()
                .map(|&s| s.to_string())
                .collect()
        )
